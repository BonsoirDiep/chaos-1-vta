    ...
    for i in range(0, len(tokens)):
        ...
        if "day" in day_tok_key or day_tok_key == "endmonth":
            ...
            for j in range(i + 1, len(tokens)):
                ...
                # print('month_tok_key', month_tok_key)
                # print('month_tok_value', month_tok_value)
                # if 1:
...
def extent_me(s1):
    ...
    for d in s1:
        ...
        x.append(s2[0])
    return x
def exist_key_in_array_dict(s0, arr):
    x1= {k for d in arr for k in d.keys()}
    x2= ' '.join(list(x1))
    return s0 in x2
...
def summary_date(message):
    ...
    # print(date_tokens, weekday_week_tokens, day_month_year_tokens)

    

    a= get_date(tokens=date_tokens)
    if  len(a)> 0:
        dates.extend([{'extent': extent_me(date_tokens), 'attribute': a}])

    b= extent_me(day_month_year_tokens)
    if len(day_month_year_tokens)>0:
        if not exist_key_in_array_dict('month', day_month_year_tokens):
            day_month_year_tokens.insert(0, {'month': 'tháng'})
        if not exist_key_in_array_dict('day', day_month_year_tokens):
            day_month_year_tokens.insert(0, {'day': 'ngày'})
    # print(date_tokens, weekday_week_tokens, day_month_year_tokens)
    a= get_day_month_year(tokens=day_month_year_tokens)
    if  len(a)> 0:
        dates.extend([{'extent': [' '.join(b)], 'attribute': [a[len(a)-1]]}])
    
    a= get_weekday_week(tokens=weekday_week_tokens, week_now=36)
    if  len(a)> 0:
        dates.extend([{'extent': extent_me(weekday_week_tokens), 'attribute': a}])
